...
public function store(Request $request){
    $validate=Validator::make($request->all(),[
        ...
        'shop_id'=> 'required|integer|exists:shops,random_id',
        ]);
        if($validate->fails()){
            return response()->json($validate->errors());
        }
          $ID=$this->getRealID(Category::class, $request->category_id)->id;
          $IDshop=$this->getRealID(Shop::class, $request->shop_id)->id;
      ...
      Shopproduct::create([
             ...
             'category_id'=> $ID,
             'shop_id'=> $IDshop,
             'image_array' =>json_encode($imagePaths),// Array of image URLs
